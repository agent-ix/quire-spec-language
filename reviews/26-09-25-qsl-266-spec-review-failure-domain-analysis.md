...
| --- | --- | --- |
| FND-001 | fixed accbac38 | The claim is now "defined and its result lies in its result bound, for every assignment of its extent roots under which its path condition holds". The path condition is made of the enclosing `if` conditions (true in `then`, false in `otherwise`) and the short-circuit left operands (true for `and` and `implies`, false for `or`), outermost first. Parameters and binders read by a guard are roots. FR-057, FR-062 and ADR-014 agree, and RR-11, RR-12 and RR-13 pin it: the `+` in RR-12 has guard `n >= 0 and n < 10` and is `Unbounded` at `n`. Filters stay out, per the leader's ruling that a path condition equals what `check::facts` relies on (FR-057:171, FR-062:217-232, 251-259, ADR-014:236-248). |
| FND-002 | fixed accbac38 | FR-062 now says `check` "SHALL pair each site with the occurrence recorded at that site's own `Location`, never by the order in which sites or occurrences were produced". Each record carries its own occurrence's extent, result bound and path condition. RR-15 is the sibling-`let` fixture, and TC-160 step 10 checks it in both operand orders (FR-062:239-249, 263-266, 304; TC-160:72-76, 125-128). |
| FND-003 | fixed accbac38 | A claim site is now an occurrence "whose region lies inside the function's body", and "An occurrence in the function's `decreases` measure is not in the body". RR-17 pins one record at the body's occurrence and none at the measure's (FR-062:205-208, 306). |
| FND-004 | fixed accbac38 | FR-062 now says every scalar operation application in a body has at least one `expression` occurrence. A site with none, including a `generated`-only application, is `KeyFault::UnkeyableRequirements` with no checked package (FR-062:268-273). |
| FND-005 | fixed accbac38 | `check` SHALL classify each claim's extent while it checks the declaration, under its stage limits. Reaching the node-count ceiling is the declaration's `StageFailure::Limit` of kind node count, located at the declaration, and a missing composite is an internal fault. The ADR-012 sketch says extents are classified in `check` (FR-062:275-281, ADR-012:237). |
