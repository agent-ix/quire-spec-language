...
| FND-010 | low | Second disposition pass, at 1d69b5ee. FR-075 says the bounded follow-up "carries the original item's occurrence key" and that the driver joins its settlement to the original record by occurrence key. But it is written by `RequestWriter::bounded_item(node, requirements, bounds)`, whose `RequestItem` (qsl-route/src/request.rs:98-104, 205-210) has no occurrence key. Nothing states whether `bounded_item` gains an occurrence-key parameter or the driver pairs the key itself. There is also a second gap. FR-097-AC-3 allows several bounded items for one domain set with different bounds, but a join "by occurrence key" maps two such follow-ups to one record, and nothing states what the driver does with two settlements for one record. State where the occurrence key enters the follow-up item, and either one follow-up per record or how their settlements combine. | spec/functional/FR-075-compute-candidates-from-registered-backends.md:145-152; spec/functional/FR-097-classify-claim-extent-and-write-bounded-requests.md:71 |
| FND-011 | low | Third disposition pass, at 394e9c22. FR-075-AC-8 now includes a driver clause, "the driver writes no second follow-up for that record", but its Verification column names only TC-449, which runs QSL's builder and `RequestWriter` and cannot observe the driver. Per the leader's ruling, TC-449 asserts a single item at request level, and the at-most-one-follow-up rule is driver behaviour to be tested in quire-driver when QSL-1 drops `RequestedItem`. The AC does not say so. As written, the clause claims QSL-local evidence it has none of. Either move the clause out of AC-8 into the FR-075 driver prose, or name its owner in the row (quire-driver test, QSL-1) so the trace does not overstate TC-449. | spec/functional/FR-075-compute-candidates-from-registered-backends.md:155-158, 249; spec/test-cases/TC-449-request-builder-writes-one-item-per-requirement-record.md:30-36 |
