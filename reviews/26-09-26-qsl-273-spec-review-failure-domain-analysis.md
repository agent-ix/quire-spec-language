...
| FND-008 | fixed f83ca481 | FR-104 and ADR-012 §15.7 key a population domain `DomainKey{node: member object_type node, path: [population ordinal]}` (path is `Vec<u32>`), name the context population as the one unbounded population of the type, refuse `ambiguous_declaration`/`ambiguous-name` at `on` when there are several, and take a run's population from the selection's `self`. FR-104-AC-5 and TC-461 step 5 add the bounded and unbounded `archive` cases. |
| FND-009 | fixed f83ca481 | FR-104 gives a reference-typed operation parameter the invocation's pre observation and states that `pre(e)` cannot retag it, citing QSpec `state-contract.md`. FR-104-AC-8 and TC-459 step 5 check `deref(target).versionNumber` reads pre and `pre(deref(target).versionNumber)` refuses `wrong_snapshot`/`forbidden-pre-read`. |
| FND-010 | fixed f83ca481 | Check 11 runs per population in document order, and within one follows `enforce_frame`: creates and `FrameTypeChanged` in key order, then deletes, then fields, then the delta. FR-106-AC-7 and TC-465 rows 39 and 40 cover the type change and the population order. |
