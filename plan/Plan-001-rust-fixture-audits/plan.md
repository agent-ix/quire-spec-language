...
| TC-004 | Model checkpoint identity and producer pin | Run model-bytes over real repository fixtures, then change an isolated artifact and the producer revision independently. Five digests pass only for selected bytes/pin; mutations refuse; original bytes remain unchanged. |
