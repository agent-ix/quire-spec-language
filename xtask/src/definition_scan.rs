...
mod tests {
    ...
    /// TC-271/FR-089-AC-1: QSL-166 deleted `value::accounting`'s
    /// byte-identical duplicates of `struct Meter`, `struct Incomplete`,
    /// `enum LimitKind` and `enum ChargePoint`, repointing every consumer at
    /// `quire_exact::{..}` directly (no re-export shim). This is a
    /// regression guard, not a one-time diff: it scans `src/value/`
    /// specifically (`scan_crate`, the same machinery TC-170/TC-261 use
    /// above, restricted by file prefix) for a defining location of any of
    /// the four names, so a future change that resurrects
    /// `value/accounting.rs`, or adds an equivalent copy under another
    /// `value::` module, fails this test the same way TC-170 fails on a
    /// resurrected `value::expression::check` module.
    ///
    /// Scoped to `src/value/`, not the whole crate: like this module's own
    /// `assert_defined_exactly_once_under_check` (see its doc, above), a
    /// crate-wide-by-bare-name scan over-claims here. All four names have
    /// pre-existing, unrelated private namesakes elsewhere in this crate --
    /// none of them a copy of the kernel type, none of them QSL-166's
    /// concern -- confirmed by running this scan unscoped and finding
    /// `struct Meter` in `src/checking/proof.rs`, `src/package/encoding.rs`,
    /// `src/package/intake.rs` and `src/protocol_artifact/encoding.rs` (each
    /// a module-private JSON/proof-budget tracker), plus `pub enum LimitKind`
    /// in `src/command.rs` and `pub struct Incomplete` in
    /// `src/temporal/result.rs` (each an unrelated, pre-existing public
    /// type). `src/model/accounting.rs`'s own `Meter`/`Incomplete`/
    /// `LimitKind` are the same kind of legitimate namesake, kept
    /// deliberately separate by the QSL-164 ruling (2026-09-21: layer-3
    /// `model`'s own meter, not a kernel type) -- `src/model/` is outside
    /// `src/value/`, so it needs no explicit exclusion here.
    #[trace("TC-271", "FR-089-AC-1")]
    #[test]
    fn no_duplicate_accounting_kernel_types_in_value_module() {
        let definitions = scan_crate(&workspace_root()).expect("scan runs");
        for name in ["Meter", "Incomplete", "LimitKind", "ChargePoint"] {
            let locations = definitions.items.get(name).cloned().unwrap_or_default();
            let under_value: Vec<&Definition> = locations
                .iter()
                .filter(|location| location.file.starts_with("src/value/"))
                .collect();
            assert!(
                under_value.is_empty(),
                "{name} is defined under src/value/ (should be quire_exact's \
                 only, with no QSL-owned copy): {under_value:?}"
            );
        }
    }
}
