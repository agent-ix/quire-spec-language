...
| D24 | Convert `Rational[0,2;1,3]` value `2/3` into `Decimal[0,100;2,2;nearest-even]` with `ScalarLimitsV1 { integer_bits: 9, decimal_digits: 3, scale_expansion: 2, text_input_bytes: 0, text_scalars: 0, normalized_scalars: 0, unit_edges: 0, value_occurrences: 2, work_units: 5, result_units: 1 }`; then `scale_expansion: 1`; then into `Decimal[0,100;2,2;exact]` with the first tuple | the division of `(2,0)` by `(3,0)` into scale 2: `decimal.operands` (`integer_bits = 2`, `decimal_digits = 1`, `value_occurrences = 2`), `decimal.scale-expansion` (`scale_expansion = 2`, `sbits(2,2) = bits(2) + bits(100) = 9`, `sdigits(2,2) = 3`), `decimal.arithmetic` on `N/D = 200/3` (`integer_bits = 9`, `decimal_digits = 3`), `decimal.rounding` (9, 3) and `decimal.result-retain` (`integer_bits = 7`, `decimal_digits = 2`): result `(67,2)` with `{ exact_numerator: 2, exact_denominator: 3, rounded_coefficient: 67, rounded_scale: 2, mode: nearest-even }`, five work units and one result unit; `incomplete { limit_kind: scale_expansion, limit: 1, consumed: 0, next_charge: 2, charge_point: decimal.scale-expansion }`; `refused` after `decimal.arithmetic` with three work units, no `decimal.rounding` charge and no value |
| D25 | Type-check, with `type P = Decimal[0,1000;2,2;exact];`, `type R = Decimal[0,1000;1,1;exact];` and parameters `p: P`, `r: R`: `let v = decimal(15,1) + decimal(225,2) in v`; `decimal(15,1) = decimal(150,2)`; `p + decimal(15,1)`; `p + r`; `p + convert<P>(r)`; a `P` result body `decimal(15,1) + decimal(225,2)` | `refused { code: ill_typed, cause: ambiguous-literal }` before any charge; the same; admitted with the literal typed `P` (target scale 2); `refused { code: ill_typed, cause: type-mismatch }`; admitted; admitted, both literals typed `P` by the declared result |
