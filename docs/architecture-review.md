...
| High, next stage | Full source refs, numeric IR revision mappings, dependency closure and runtime identities are not represented by parser metadata alone | Reviewed; a separate registered interchange SourceRef contract is required and is not accepted here. Parser identity/revision labels remain diagnostic metadata only. No invented hashes/revisions or permissive fixture reader. |
...
| High | A single `profile` field meant fixture encoding in A's packet and language semantics at the portable consumer boundary | Reviewed; a single `profile` field still conflates fixture encoding with language semantics at the portable consumer boundary, and that distinction is not resolved here. |
...
| High, next stage | The initial semantic prose left contextual `size` typing, captured values inside `pre`, and exact evaluator accounting implicit | Reviewed; a separate typing/evaluation contract for `size` and `pre`-captured values is required and is not accepted here. The syntax check accepts 50 expressions and refuses collect; type/evaluation expectations remain unexecuted. |
