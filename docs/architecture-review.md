...
| High, next stage | Imported names currently have declaration/clause-wide syntax loci; precise unresolved-name diagnostics need individual reference loci | Add spanned identifier/import references when implementing the reviewed linker contract. Preserve the original document now so locations are recoverable. |
