...
| High | A single `profile` field meant fixture encoding in A's packet and language semantics at the portable consumer boundary | FS02 now proposes separate byte artifact refs and semantic refs, with independent wire, language, semantic profile and canonical identity axes. New role kinds require a new explicit wrapper version; old evidence is preserved. B/C adoption is pending. |
...
| High, next stage | The initial semantic prose left contextual `size` typing, captured values inside `pre`, and exact evaluator accounting implicit | FS03 now proposes explicit judgments and 51 independently authored cases. These refinements have their own review identity/digest and do not silently extend the earlier profile. The syntax check accepts 50 expressions and refuses collect; type/evaluation expectations remain unexecuted. |
