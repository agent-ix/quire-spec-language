...
use super::outcome::{identity_string, EvalHalt, Refusal, StateModelUndefined, Stop};
...
fn invariant() -> EvalHalt {
    EvalHalt::Stop(Stop::Refused(Refusal::CheckedInvariant))
}

/// ADR-013 T-6/FR-090-AC-8: `model::population` refused the query -- carried
/// in `FamilyResult::Refused` with `ModelRefusal`'s own `catalog_code()`,
/// never QSL's now-removed kernel-shaped `Refusal::Model`.
fn model_refusal(refusal: ModelRefusal) -> EvalHalt {
    EvalHalt::Family(FamilyResult::Refused(Box::new(refusal)))
}
...
/// checked invariant).
fn to_object_reference(key: &ReferenceKey) -> Result<ObjectReference, EvalHalt> {
...
fn resolve_target(
    ...
    target: NodeKey,
) -> Result<DeclarationKey, EvalHalt> {
...
pub(crate) fn evaluate_lookup(
    ...
    match lookup(binding, &target, &lookup_key, absence, meter) {
        ...
        // ADR-013 T-6/FR-090-AC-12: a `StateModel` undefined cause, carried
        // in `FamilyResult::Undefined` -- never the removed kernel-shaped
        // `Undefined::AbsentKey`. `binding`/`key` are the catalog's own
        // payload for reason `absent-key`.
        LookupOutcome::Undefined => Err(EvalHalt::Family(FamilyResult::Undefined(Box::new(
            StateModelUndefined::AbsentKey {
                binding: binding.population_id().to_string(),
                key: identity_string(&lookup_key.object),
            },
        )))),
...
fn option_payload(result_type: &ValueType) -> Result<ValueType, EvalHalt> {
    match result_type {
