...
| --- | --- | --- |
| [FR-001](functional/FR-001-read-exact-source.md) | FR | Draft; AC-5 to AC-10 (the caller-named four-label `RawSourceRef` for every source admission, S0 refusal regions, render-time line and column; TC-424) implemented under QSL-233 (ADR-013 §7 S-4b) |
...
| [FR-010](functional/FR-010-report-native-outcomes.md) | FR | Draft; AC-11 (the four source labels, TC-425) implemented under QSL-233 (ADR-013 §7 S-4b) |
...
| [FR-018](functional/FR-018-construct-native-runtime-inputs.md) | FR | Planned LC03 byte-bound input construction; FR-018-AC-8 (four source labels, TC-431) implemented under QSL-233 (ADR-013 §7 S-4b) |
...
| [FR-031](functional/FR-031-run-extracted-native-source.md) | FR | Implemented; tested in the native-workflow test matrix; FR-031-AC-5 (four source labels on the body record, TC-430) implemented under QSL-233 (ADR-013 §7 S-4b) |
...
| [FR-056](functional/FR-056-admit-domain-package-model-declarations.md) | FR | Planned domain-package model intake; #131; the compiled-protocol `Model` naming this admits directly is specified under #132 (FR-042-AC-11, FR-042-AC-12); FR-056-AC-9 (spine I1 and model type names in the assembler, TC-442) implemented under QSL-249 |
...
| [FR-071](functional/FR-071-implement-typed-replay-request.md) | FR | Implemented under #231 (QSL-88); TC-185, TC-186 (step 7, FR-071-AC-9, under QSL-255), TC-188 pass locally, TC-187 stays Planned (absence-only claim not runtime-assertable) — see `spec/tests.md` |
...
| [FR-075](functional/FR-075-compute-candidates-from-registered-backends.md) | FR | Specified under #185 (QSL-185); implemented under QSL-46 (PR #305) as `src/route.rs`, now the crate `qsl-route` (QSL-184, `qsl-route/src/lib.rs`), over the canonical `Capability` type (QSL-173); AC-1..AC-5 backed (TC-193..TC-196); the lowering-target catalog is deleted with `lowering` in ADR-011 §7.3 M-6a |
...
| [FR-087](functional/FR-087-typestate-and-cross-package-node-key.md) | FR | Specified under QSL-158 (ADR-013 §7 S-3a: T-1, T-3, O-15), split from QSL-158's S-3 by the 2026-09-21 ticket comment; partly implemented (PR #304: `CheckedGraph`, `CheckedPackage`, `EmittedPackage`; PR #299: `library` relocation, `PackageNodeKey`; AC-5 and AC-12 backed). Gate (S-2, QC-10) is clear. Owner ruling 2026-09-21: `CheckedPackage` is canonically layer-4 `package`, `check` builds `CheckedGraph` (its own S3 output) instead; closes the `CheckedPackage`-placement half of QSL-167. AC-13 (E3 imported-name resolution and its missing/ambiguous refusals, TC-379) is unbacked |
| [FR-088](functional/FR-088-clause-name-and-type-identity.md) | FR | Specified under QSL-158 (ADR-013 §7 S-3b: O-08, O-09 clause id, O-10, O-11, O-14, C-26), split from QSL-158's S-3 by the 2026-09-21 ticket comment; S-3b implemented by #300 (see its Status); FR-088-AC-11 (FR-141 `VariantId` and rank key, TC-409) and AC-12 (two-domain `UnitId`, TC-411): QSL-131 |
...
| [FR-090](functional/FR-090-return-a-family-outcome-or-a-typed-family-refusal.md) | FR | Implemented under QSL-174 and QSL-191 (ADR-013 O-16, O-17, T-4, T-6); TC-382, TC-384 to TC-391, TC-407 and TC-408 passed locally |
| [FR-091](functional/FR-091-produce-value-forms-and-assemble-package-declarations.md) | FR | Specified for ADR-011 §7.3 M-3b (`Value` family, QSL-141) on the M-6a path (QSL-8); not yet implemented -- TC-392–TC-403, TC-405, TC-406, TC-412 planned. OQ-1 to OQ-5 and OQ-7 ruled (see Rulings). Open questions FR-091-OQ-9 (type-form bounds as kernel values or spelled text), OQ-10 (`unsupported_construct` for S2's unrepresented-form causes) |
...
| [FR-105](functional/FR-105-emit-state-nodes.md) | FR | Specified under QSL-273 (A05-3): S4 `state`/`state_clause`, `operation_anchor` and `frame` nodes; QSpec body rule and `reaches` field member are prerequisites; not yet implemented -- TC-462, TC-463 planned |
...
| [FR-108](functional/FR-108-run-the-configversion-spine-corpus.md) | FR | Specified under QSL-273 (A05-3): the ConfigVersion spine corpus with native-equal typed dispositions (QSpec FR-180-AC-1, AC-4, AC-5, QSL's share); not yet implemented -- TC-469 planned |
| [FR-109](functional/FR-109-run-a-state-clause-through-the-spine.md) | FR | Specified under QSL-273 (A05-3): layer-6 `qsl_replay::spine::run_clause`: compile, select, admit, evaluate, report; not yet implemented -- TC-468 planned |
