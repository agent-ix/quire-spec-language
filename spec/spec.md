...
| [FR-060](functional/FR-060-check-qsl-api-surface-boundary.md) | FR | Implemented ADR-011/ADR-013 T-12 API-surface boundary check (`arch-lint api-surface`); T12-A pending, T12-B live with a `check`-only allow-list and a named debt list that only shrinks (amended 2026-09-22; gate rewrite pending), T12-C live and failing at OBS-018, T12-D (kernel `PopulationId` constructor, `model` only) live and passing; #215 |
