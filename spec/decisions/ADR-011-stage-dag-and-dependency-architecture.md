...
- **K is a leaf.** `quire-exact` depends on no QSL module and no ecosystem
  ...
  (`value/outcome.rs:10,13`). `collection` and `equality` import `key`,
  `enumeration` and `quantity`. X-1 must cut every
  ...
  by QC-15, QC-21 and QC-22 (TK-10; ADR-013 §8): the `Reference`, `Quantity`
  ...
  **This requirement was not met by the change that was supposed to meet
  ...
  QSL-131 lands, `collection`, `equality`, `composite` and `outcome`
  continue to import `key`, `enumeration`, `quantity` and `reference` from
  `value` — an
...
- **`model` sits below `check`.** M-2 moves the two things that make `model`
  ...
  change; QSL-131 has cut the edges named above (composite/collection/
  equality/outcome are all deleted), not M-2 or M-5. After M-2 and
...
| `value` kernel submodules: `numeric`, `integer` (QSL's copy is a `value/mod.rs` `pub use` re-export, not a module; QSL-146 owns it), `rational`, `decimal`, `ieee` and `division` (operations), `text`, `collection`, `comparison`, `equality`, `outcome`, `accounting`, `composite` | K `quire-exact` | only the types in the AD-016 Shared-type row as amended by QC-15, QC-21 and QC-22 (TK-10; ADR-013 §8), and the operations over them. #213 S-1 (X-1) added these as new `quire-exact` types; its own PR summary states no file under `src/value/` or `src/model/` was touched (§6.1's "K is a leaf" bullet), so removing the matching QSL copies was a separate step. That step is now done: `accounting` is gone (QSL-166), `outcome` is gone (QSL-131 O2), `division` is gone (QSL-131 K2, #339; QSL calls `quire_exact::divide`/`modulo` directly), `decimal`, `ieee`, `numeric` and `text` are gone (QSL-131 O3), and `collection`, `composite`, `equality` and `rational` are gone (QSL-131 V5b). QSL-131 V5b also widened `quire_exact::collection`'s `form` and `member_equal` from `pub(crate)` to `pub` (as `Outcome`-returning wrappers, the same split its own `form_grouped` already used), the last "operations over" the K row's shared types QSL's evaluator needed but could not previously reach; no `value::` copy is left under `src/value/` for any of these twelve former submodules. |
...
original scope): it needed the K-designated `value` submodules (`composite`,
`collection`, `equality`, `outcome`) to stop importing the non-kernel
submodules it takes (`enumeration`, `quantity`, `key`, `reference`;
`definition` is imported by none of them, and `unit` is clean, imported only
by `quantity`, which moves with it). Cutting those edges was QSL-131's scope
and is now done: `outcome` was deleted by QSL-131 O2, and QSL-131 V5b
deleted `composite`, `collection` and `equality`, the last three (`value`
non-kernel submodules now import only `quire_exact` directly, never a
`value::composite`/`collection`/`equality` sibling). A different edge
remains, the one `semantic_value` itself would create by depending on those
K types' current, not-yet-relocated location (that direction self-heals
when `quire-exact`'s items physically move; this direction did not, until
now — §6.1's "K is a leaf" bullet).
...
| QSL-165 | Create `semantic_value` from `value`'s six non-kernel submodules (`definition`, `enumeration`, `unit`, `quantity`, `key`, `reference`); split out of M-2's original scope (2026-09-20) | 3 | `semantic_value` | QSL-131 has cut the edge this change was blocked on: `composite`, `collection` and `equality` are deleted (QSL-131 V5b) and `outcome` was deleted earlier (QSL-131 O2), so none of the four K-designated `value` siblings that used to import the six non-kernel submodules back still exist (§6.1's "K is a leaf" bullet). Independent of QSL-131 → QSL-165 scheduling. | none |
