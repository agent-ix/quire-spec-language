...
|---|---|---|---|
| S0 Source | Source bytes with `SourceIdentity` (authority, identity, revision namespace and value; FR-001) | QSL `source` | none |
...
| `profile_selections` | The clause profile rows FR-322 defines (`temporal_profile`, `protocol_profile`); empty for a package whose declarations are all `Value`-family. E3 resolves each source header `profile … version … digest …` declaration against QSL's `DefinitionLock` catalog: it selects the `root` row (`quire.value.complete/v1`) by identity, revision value and digest, or refuses (FR-110, amended 2026-09-26). |
...
A header profile selects the catalog's `root` row exactly. Otherwise E3
refuses it `unknown_profile` (`unsupported-selection` for an identity in no
row, `wrong-selection-role` for another row's identity) or
`stale_dependency` (`revision-mismatch`, `byte-digest-mismatch`). A `model`
...
that function once FR-110 is implemented. Its closure over a caller-supplied
`DefinitionCatalog`, its complete-V1 bundle, its `PackageLimits` and its
compiled-model catalog retire with it: only a test fixture can build their
inputs, and the catalog rows carry no edges, bytes or facets (FR-087
Behavior).
...
- The S4 in-process type is defined in layer-4 `package`, the crate
  `qsl-package` (X-7, §6.2). `package` keeps its fields and its constructor
  private and exposes read-only accessors. A `compile_fail` test shows that
  code outside `qsl-package` cannot construct it. The AD-016 arrow 7
  executor is `CheckedPackage::call`, over the one type
  `qsl_package::CheckedPackage`. **Amended by QSL-182 (X-7).** This bullet
  used to have `value::expression` name the type through one `pub use`, so
  that `value::CheckedPackage::call` was the path. Once `CheckedPackage` is
  in `qsl-package`, that line is a root-crate re-export of a moved item,
  which §7.2 and QSL-182's acceptance criteria forbid. Callers name the type
  at `qsl_package::CheckedPackage`; `CheckedPackage::call` still resolves
  through `CheckedPackageEvaluation`, which is the contract the path stood
  for. QSL-181 resolved FR-068-AC-10 the same way (ruling R3).
- `CheckedPackage`'s layer-5 operations, `call` and `evaluate`, are the
  methods of the extension trait
  ...
  them, at `qsl_eval::value::CheckedPackageEvaluation`: layer 5 is the crate
  `qsl-eval` (X-8, QSL-183), whose `value` module re-exports it.
...
| SEAM-4 IT-010 | `tests/it/configversion_backends.rs` proof and replay path, QSL dev dependencies on CG 5e2a6a9 and IR 04eb6f8, and the RT test fixture crate | Deleted with `lowering` and `lower` in M-6c (owner ruling 2026-09-24), so CI keeps a Kani proof-and-replay check throughout | M-6c |
...
| `complete::package` (import resolution) | 3 `library` | resolves imports against I2 import views; `lower_source_graph` is SEAM-5. `resolve_source_package` takes a source's authority and its F `SourceSelections`, not a `ParsedSource`; parsing, and refusing an inadmissible parse, is its caller's step, shipped as layer-6 `command::resolve_parsed_source`, which derives the authority and the selections from one admitted parse; a resolution refusal carries layer 3's own `ResolutionCause` (QSL-181). |
| `qsl_cst::cst` selection values (`DefinitionDigest`, `DefinitionRef`, `InvalidDefinitionComponent`, `ModelDigest`, `ModelRef`, `InvalidModelComponent`, `ProfileSelection`, `ImportSelection`, `ModelSelection`, `SourceSelections`) and `complete::package`'s `ProfileCatalog`, `ProfileStatus` and `StaleProfile` | F `selection` | QSL-181: plain values that layer 1 produces, layer 3 resolves and the tool modules check profiles against. None of those layers owns them, and none may import another's, so they sit below all three. This keeps the tool row (`1, F`) true: `parse_with_catalog`, `complete::editor` and `complete::edit` use F's `ProfileCatalog`. |
...
| `check::node_key` | 3 `check` | the FR-322 checked application-node key, the FR-092 structural-node key and recursion-group keying (QSL-156). It imports `check`'s `MAX_CHECKING_DEPTH` as its body-depth bound, so it cannot sit below `check` core. The FR-093 lowering of checked expressions to FR-322 nodes sits beside it in `check::lowering` (QSL-156 A4b); the `package` v2 emission arm serializes those nodes and lowers nothing (QSL-6 S1b). |
...
| `family` | 3 `check` core | the family contract (`FamilyKind`, `FamilyContract`), the S1-S4 outcomes and the S6a result types (`FamilyOutcome`, `FamilyResult`, `EvalOutcome`). The S6a hook `ReferenceEvaluation` and the S6a family kind (`S6aFamilyKind`, FR-090-AC-4) are 5 `value::expression` (`value::expression::s6a`, QSL-181): ADR-012 §2 has one marker type implement both `FamilyContract` and `ReferenceEvaluation`, and the orphan rule (E0117) needs that impl in the crate of the trait or the type. `ReferenceEvaluation`'s implementation needs the evaluator's environment, so the trait sits with the evaluator and the marker type (`check::ValueFunctionFamily`) stays in layer 3. |
...
| `value::expression::evaluate`, `value::expression` `CheckedPackage::call` | 5 `value::expression` | S6a and the replay executor, at the AD-016 arrow 7 path. Layer-5 `value::expression` is the crate `qsl-eval`'s module `value::expression` (X-8, QSL-183); `CheckedPackage::call` is `qsl_package::CheckedPackage`'s, through `qsl_eval::value::CheckedPackageEvaluation` (§4). |
...
| M-2 | Move `model` below `check` (§6.1): move `model::checked_dispatch` and `model::conformance::check_field_refinement_obligation` (with its seven exclusive helpers and one exclusive const -- `CLAUSE_SELF_SLOT`, `clause_location`, `self_field_node`, `field_domain_type`, `presence_condition`, `comparison_condition`, `established_facts` and `format_interval`) to `check`, which (sitting above `model`) reads `model::index::ModelIndex` (`pub`, with `pub(crate)` `field`, `operations` and `scalar_bounds` accessors) and calls back into `model::conformance::missing_member` (`pub(crate)`) and `AxisFailure`/`ConformanceOutcome` (`pub`) | 3 | `model`, `model::intake` | after M-5 (QSL-139) (corrected 2026-09-20; was "after X-1") | none |
...
| X-6 | Extract crate `qsl-semantics` (QSL-181). **Extracted** (X-6b): `check` (with its family checkers), `family`, `library`, `model` (with `model::intake`), `complete::package` (as `qsl_semantics::complete`) and the layer-3 `value` submodules (`definition`, `enumeration`, `unit`, `quantity`, `containment`, `declaration`, `stop`, `member`, `semantic_node`, `model_query`) are in the crate `qsl-semantics`. Its `[dependencies]` name `qsl-forms` (2), `qsl-foundation` (F) and `quire-exact` (K) among the workspace crates, and the FCD crates, which only `model::intake` names; TC-390's dependency check enforces both. The root crate kept `value::expression` (layer 5) in its own `value` module until X-8 moved it to `qsl-eval`. The tests that exercise only layer 3 are in `qsl-semantics/tests/`. `qsl-semantics`' test-only fixtures are behind its `test-support` feature, which only a `[dev-dependencies]` entry enables. | 3 | the layer-3 modules' public items | after X-5, on the same condition | none: no root-crate re-export (§7.2) |
| X-7 | Extract crate `qsl-package` (QSL-182). **Extracted**: layer-4 `package` — `CheckedPackage`, `EmittedPackage`, the v2 emitter (`emit`) and the I2 byte reader (`checked_v2`) — is the crate `qsl-package`. Its `[dependencies]` are exactly `qsl-semantics` (3), `qsl-foundation` (F), `quire-exact` (K: the kernel occurrence `Origin` the I2 reader keys the v2 source map by, ADR-013 O-07, QSL-159), `quire-contract-model` (the v2 wire contract: IR's typed structs the emitter builds and IR's reader the I2 reader delegates to), `quire-canonical` and `thiserror`. Its `[dev-dependencies]` may name layer 2 (`qsl-forms`), for its tests only. TC-390's dependency check asserts the exact `[dependencies]` set and refuses any other workspace crate in the dev and build tables. It has no `test-support` feature. The SEAM-1 `package` submodules stay in the root crate until M-6. The root crate re-exports none of its items, including `CheckedPackage` (§4, amended). | 4 | the layer-4 modules' public items | after X-6, on the same condition | none: no root-crate re-export (§7.2) |
| X-8 | Extract crate `qsl-eval` (QSL-183). **Extracted**: layer 5 — the S6a evaluator `value::expression` (with `causes`, the `s6a` seam and the `Value` family evaluator) and `simulation` — is the crate `qsl-eval`, at `qsl_eval::value` and `qsl_eval::simulation`. Its `[dependencies]` are exactly `qsl-package` (4), `qsl-semantics` (3), `qsl-foundation` (F), `quire-exact` (K), `quire-canonical`, `serde` and `thiserror`. `quire-canonical` and `serde` encode FR-101's simulation state key and sampler preimage through ADR-013 §2's one RFC 8785 encoder, which also takes their SHA-256, so `sha2` stays a dev dependency (QSL-272). Its `[dev-dependencies]` may name layer 2 (`qsl-forms`), layer 1 (`qsl-cst`) and `serde_json`, for its tests only. TC-390's dependency check asserts the exact `[dependencies]` set and refuses any other workspace crate in the dev and build tables. It has no `test-support` feature. The old `state` and `temporal` evaluators are SEAM code and stay in the root crate until M-6c; `qsl-eval` imports neither. No shipped root-crate code calls a layer-5 item yet, so the root crate does not depend on `qsl-eval` in any table, which TC-390 asserts. The tests that exercise only layer 5 and below are in `qsl-eval/tests/`. | 5 | the layer-5 modules' public items | after X-7, on the same condition | none: no root-crate re-export (§7.2) |
| X-9 | Extract crate `qsl-route` (QSL-184). **Extracted:** `route` is the crate `qsl-route`, with its items at the crate root (`qsl_route::Registry`, `BackendDescriptor`, `CandidateOutcome`, ...). Its `[dependencies]` name `qsl-semantics` (3) and `qsl-foundation` (F) among the workspace crates; `route` imports nothing of layer 4 or K, so it did not wait for X-7. TC-390's dependency check enforces the list. No shipped root-crate module calls `route`, so the root crate names `qsl-route` only as a dev dependency (§7.1), and the `xtask seam-probe` S7 build compiles `qsl-route` separately. The tests that exercise only `route` are in `qsl-route/tests/`. | R | `route` | after X-6; X-7 is not a prerequisite, since `route` imports no layer-4 item | none: no root-crate re-export (§7.2) |
...
|---|---|---|
| M-6a checked-package producer | CLI `compile` routes by source edition: `1-draft` → spine (S1–S4), `0-draft` → native until M-6c/QSL-5; native compile/lower are deleted with M-6c and QSL-5 (owner ruling 2026-09-24). `format` is retargeted to the CST, and native-edition `format` retires with no replacement. That retirement is a scoped exception, by owner decision on 2026-09-22, to the 2026-09-19 ruling that nothing working is removed early: `format` is tooling with no downstream artifact, the native lane is retiring, and native `run` does not need formatted input (Rulings 2026-09-22, native-edition `format`). `lowering` (`ProjectionTarget` and `--target` included) and the IT-010 path (SEAM-4) are not in M-6a: the `lower` command moved to M-6c (owner ruling 2026-09-24), and IT-010 imports `lowering` (`tests/it/configversion_backends.rs:17`), so both are deleted with `lower` in M-6c. The QSL dev dependencies on CG, IR and the RT fixture crate are deleted with `lower` in M-6c, together with `lowering` and IT-010 | the spine for those commands: QSL-8 (this repo's #240) with M-4, before #216. Spine `compile` and spine `run` (§5) are the replacements. The skeleton spine is QSL #243 (QSL-5) with agent-ix/quire-contract-codegen#87. |
| M-6b proof and replay | nothing: its former contents, SEAM-4 and `lowering`, are deleted in M-6c | #217 widens the skeleton spine to the function-application exemplar and deletes nothing. The skeleton moves CG's dev pin on QSL from 21c507e to a QSL revision that has M-4 and the S6a entry. |
| M-6c state and temporal evaluators | `state`, `temporal`, and the SEAM-3 reads and `native_model` and IR imports that feed them; native `run` (native-run/1 clause execution over snapshots and invocations); the native `compile` and `lower` commands, with QSL-5 (owner ruling 2026-09-24); `lowering`, IT-010 (SEAM-4) and the QSL dev dependencies on CG, IR and the RT fixture crate, which `lower` and IT-010 still use; and the SEAM-1 modules only they reach, including `package::NativePackage`, the native-linked-package/1 reader, `runtime`, `mapped` and `model_source` (ADR-011-OQ-2) | #120, #121 and #164 (state; design #220) and #188 and #189 (temporal; design #222). The PR that lands spine clause execution deletes native `run`. |
...
- **OQ-3: `NativePackage`, `lowering`, `runtime` and IT-010.** `lowering`
  ...
  agent-ix/quire-contract-codegen#87 (§6.2, §7.3). Amended 2026-09-24: `lower` moved to M-6c and IT-010 imports `lowering`, so `lowering`, IT-010 and the dev dependencies are deleted in M-6c, not M-6a. `NativePackage`,
