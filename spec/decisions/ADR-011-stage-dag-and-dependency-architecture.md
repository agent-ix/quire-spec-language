...
| QSL-166 | Remove the QSL `model` and `value` duplicates of `Meter`, `Incomplete` and `LimitKind` (`model/accounting.rs:244`, `value/accounting.rs:465`), delete `model::accounting` and `value::accounting` and their `pub mod accounting` declarations (`model/mod.rs:14` and the `value/mod.rs` equivalent), and repoint the 19 importing modules at `quire-exact`'s existing kernel `Meter`, `Incomplete` and `LimitKind` | K `quire-exact`; the removed modules' importers repoint at `quire-exact` | none new: consolidates onto `quire-exact`'s existing kernel types; no widened visibility verified | after X-1; independent of QSL-131 and QSL-146 (disjoint scope — `Meter`/`Incomplete`/`LimitKind` only, not `CollectionKind`/`Integer`/`IntegerInterval`/`AbsenceMode`/`ValueType`/`Value`/`negotiate_*`) | none: no shim, no alias module, no re-export |
