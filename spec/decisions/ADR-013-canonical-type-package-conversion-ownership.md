...
| TK-08 | Model-owned node-identity vectors (QC-3), FR-321 duplicate-selection code (QC-5), FR-322 dependency node reference (QC-10), catalog codes for stage limits and internal fault (QC-11), the node-identity preimage owner scope and type-node arm (QC-18) and the unknown-node-kind code (QC-19). | QSpec |
