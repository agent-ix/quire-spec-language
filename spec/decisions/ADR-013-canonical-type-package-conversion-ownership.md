...
| C-29 | Candidate set → FR-331 `candidates` (QC-12) | QSL `route` returns the typed value; the FR-331 envelope writer serializes it; CG reads | One value per request item: the candidate list ordered by (identity, manifest digest), or the unknown-backend mark. The registry is keyed by identity, so a list holds no duplicate. QSL defines no candidate-set wire; the member layout is agent-ix/quire-specification#134's | FR-057-AC-8 and FR-075-AC-1 to AC-3 over `CandidateOutcome`; the wire round trip and CG's duplicate-member adverse test follow agent-ix/quire-specification#134 |
