...
| ADR-010 OBS-004 | `negotiate_integer_division`, `negotiate_ieee` and `IeeeBackendCapabilities` are RT capability predicates (AD-016 arrow 3). A CG `negotiate_*` arm may take them as inputs (arrow 4, WP7 open), but they are not settlement points and settle no disposition; today CG calls neither (agent-ix/quire-contract-codegen#86). CG `negotiate_*` arms over S9 are the only settlement points. The predicate list is decided by AD-016 WP7. QSL has no copy of them; #213 S-1 removes the QSL copies when X-1 moves `division` and `ieee` into `quire-exact` (ADR-011 §6.2). QSL value semantics keep their evaluation functions, which are not negotiation. |
...
| Removal of the QSL `negotiate_*` copies from `value::ieee` and `value::division` (OBS-004), when X-1 moves them into `quire-exact` | #213 S-1 |
...
| FR-063's S1-S4 seam-probe coverage of the mechanism itself (`xtask seam-probe`'s own end-to-end behavior, dedicated trace-tagged tests) (FR-063-AC-1, AC-2, AC-3, AC-4, AC-7) | [QSL-149](https://linear.app/agent-ix/issue/QSL-149) |
...
| Occurrence-span survival across S4 linking with a real, non-pass-through transformation, and the composed-checker deletion this migration's own rescoping left as a separate change (FR-065-AC-1, AC-3) | [QSL-154](https://linear.app/agent-ix/issue/QSL-154) |
| Moving function declaration/application checking (typing, definedness, termination) into `ValueFunctionFamily::check` and deleting it from `Typer`/`Self::call`, closing the pre-migration composed-checker entry points this ticket's rescoping decision left in place (FR-065-AC-4, AC-5) | [QSL-148](https://linear.app/agent-ix/issue/QSL-148) |
