...
- The checker records `Requirements` as data, in per-item requirement
  records (ADR-011 §2.2 E3). Recording a requirement grants nothing. The v2
  `capability_report` is FR-322's feature-level report, not these records.
...
| `Relation`'s absence from S6a's input type (FR-090-AC-4, backed); FR-062-AC-8's S4 cause-bearing-family seam-probe coverage (backed, `CheckCause::code`); AC-6's second sentence (every other family's `evaluate` hook reads no CST, token or display string, still unbacked) (FR-062-AC-6, AC-8) | [QSL-152](https://linear.app/agent-ix/issue/QSL-152) |
| `FamilyContract::package` (ADR-012 §2's sixth contract part), and AC-9's fault-injection behavior over it (FR-062-AC-1's `package` half, AC-9) | [QSL-242](https://linear.app/agent-ix/issue/QSL-242) |
...
| Two typing contexts constructed from the same resolved declarations, checking the same form through each, produce identical checked output (FR-062-AC-3's third clause, now backed by `TC-160`/`two_contexts_from_the_same_declarations_check_identically`; the criterion's first two clauses remain untested) | [QSL-161](https://linear.app/agent-ix/issue/QSL-161) |
