...
crate's symbols and from every Rust source and build-input file — `src/`,
`tests/`, `benches/`, `xtask/`, `examples/`, not `src/` alone, so a move of
`lower_source_graph` and its test into `tests/` or `xtask` cannot pass —
including the pre-migration test that called `lower_source_graph` directly.
This scan does not reach `spec/` or `docs/`, which name the three symbols by
design (this file among them) and are not the compatibility hole this
requirement closes. No compatibility shim, disabled test, relocated copy, or
feature-gated dormant copy of any of the three remains in the scanned code
directories. The symbol scan (step 1) is a weak oracle on its own — rustc
emits no symbol for an unreferenced non-generic struct, so it can pass while
`LoweredDeclaration` still exists in source — which is why the code-wide
...
4. Inspect the pre-migration test that exercised `lower_source_graph`
   ...
   feature gate that would disable it without removing it.
...
- Step 3: no source reference to any of the three names exists anywhere in
  `src/`, `tests/`, `benches/`, `xtask/` or `examples/`.
- Step 4: the pre-migration test is absent entirely from those same code
  directories — not present but disabled, and not present relocated to
  `tests/` or `xtask`.
