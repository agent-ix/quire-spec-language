...
10. Pass the actual production-emitted immutable artifact/reference to
    ...
    sources, admitted domain packages, registry/contract and compiler binary
    identity (a digest on `Producer.binary`, not retained bytes) to construct
    ...
    its own selections.
    Retain all four original source files and the actual executing compiler
    binary's identity from the recipe. Reconstruct its admitted model
    declarations through the same intake path; do not replace the model or
    binary with invented metadata.
