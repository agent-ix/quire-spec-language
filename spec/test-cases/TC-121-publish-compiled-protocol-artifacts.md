...
6. Bind O1 and O2 to the same provider through distinct instance requirements.
   ...
   value type and require refusal; an explicitly selected typed effect view
   retains its payload/model fields. No operation result or recovery snapshot
   supplies the missing effect identity or correspondence.
