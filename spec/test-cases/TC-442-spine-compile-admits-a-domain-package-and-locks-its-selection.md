...
1. Run `tests/fixtures/spine-model.native` (functions over `M::Gadget` and
   `Reference<M::Widget>`) with `tests/fixtures/spine-model.semantic-ir.json`
...
3. Assemble the fixture with `Reference<M::Nope>`, and admit its selections
   with no package input.
4. Compile the fixture through the CLI with `Reference<M::Nope>`; with no
...
`#[trace("TC-442", "FR-027-AC-9")]` (steps 2 and 4) and
...
- Step 1: nothing is omitted; `M::Gadget` and `M::Widget` are declared object
  ...
  the read is Verified and exports `keep` and `held`; the other digest is
  refused.
...
- Step 3: one assembler error, `UnresolvedTypeName("M::Nope")`, code
  `missing_declaration`, at `M::Nope`; intake refuses `missing_import` at the
  `model` declaration.
- Step 4: exit 20 and empty stdout each time; stage `assembly` with
