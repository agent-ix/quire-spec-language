...
3. Supply no library; supply `test/geometry` at version `2`; supply two
   libraries as `test/geometry`; supply `test/a` and `test/b`, each
   importing the other, and import `test/a`.
...
5. Declare `function p using v(y: Int[0, 9]): Boolean pure { g::f(y) }`
   and compile; then replace the call with `g::f(true)`.
...
- Step 2: `invalid_digest` at S1 at the digest string; then
...
- Step 3: `missing_import`/`missing-selection` at the import;
  `stale_dependency`/`revision-mismatch` at the import;
  `invalid_package`/`conflicting-definition` from the dependency input;
  `invalid_package`/`definition-cycle` naming `test/a` and `test/b`. No
  package.
...
- Step 7: stdout is exactly the bytes spine `compile` returns for step 5's
  unit and library, exit 0; the `0-draft` request refuses with
  `invalid-request`, exit 20, empty stdout.
