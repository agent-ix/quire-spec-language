...
1. Run a native-run/1 request selecting `tests/fixtures/spine-compile.native`,
   ...
   same source with `qsl_replay::spine::compile` under the same labels.
2. Run the `0-draft` standalone fixtures' native-run/1 requests and compare
   stdout and exit status with the native run results TC-105 fixes.
...
- Step 1: exit 0; stdout is one newline-terminated `spine-run-result/1`
  document whose `outcome` is
  `{"kind": "completed", "value": {"kind": "integer", "decimal": "7"}}`,
  whose `function` is `seven`, whose `package_id` equals the compiled
  package's, and whose `source` names the request's four labels and source
  digest.
- Step 2: stdout bytes and exit status equal the native run results.
...
- Step 5: each refuses `invalid-request`, exit 20, empty stdout.
