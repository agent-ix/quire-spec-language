...
6. Construct a sum value from the converted type and confirm it carries its
   `VariantId`, never a bare integer variant index; reorder the sum's
   declared variants (if the source syntax admits it) and confirm the
   `VariantId` computed for each variant is unchanged (position-independent,
   since it is a digest over the declaring sum and the member, not an
   index).
...
- Step 6: sum values carry `VariantId`, never an index; reordering does not
  change any variant's `VariantId`.
