...
3. Activate one instance, then mutate the source observation's amount, its
   ...
   record. Pair this with a compile-fail doctest establishing that the public API
   exposes no mutable path to a retained capture record.
...
7. Supply a trigger whose activation guard evaluates false and require no
   ...
   trigger is guard-false and require `inactive`. Supply a guard whose input is
...
8. Compile a declaration activating on a whole-execution origin and drive it with
   ...
   subject and that origin identity. Drive it with two admitted executions and
   require two instances that do not alias and whose captures do not satisfy each
   other. Re-evaluate one execution incrementally and require its instance count
   to stay one.
9. Activate an instance that remains unsettled, force eviction of one retained
   capture through the evaluator's own retention ceiling, and require an
...
recomputed afterwards. `inactive`, `unknown`, incomplete, refused and a false
guard remain five distinct activation outcomes, and none of them is a temporal
truth.
