...
extent classification and candidate outcome. Scope: FR-075-AC-8.
...
2. Check `function inc using v(n: Integer): Integer pure { n + 1 }` and run
   the builder over its records with the same registry.
...
- Step 1: three items, request indices 0, 1 and 2, in the bytewise order of
  ...
  `bounded`, and its candidate set is exactly the one backend.
