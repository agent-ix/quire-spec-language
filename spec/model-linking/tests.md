...
| TC-121 | Full compiled protocol artifact and Rust handoff requiring [B's IT-001](ix://agent-ix/quire-protocol/IT-001) | Integration | P1 | FR-042-AC-1..FR-042-AC-11 | 🚧 QSL `/1` publication and strict read passed locally (`tests/published_protocol_v1.rs`, `src/protocol_artifact/handoff.rs`); Protocol #11 consumer acceptance pending; FR-042-AC-11 planned under #132 |
...
cross-repository Protocol leg remains open. FR-042-AC-11 (the domain-package
`Model` shape, agent-ix/quire-spec-language#132) carries no tag yet: it is
planned, not backed. One module carrying each criterion's tag:
