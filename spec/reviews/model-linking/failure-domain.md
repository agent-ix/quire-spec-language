---
id: SR-031
title: "failure-domain review of LC02 evidence design"
type: SpecReview
...
The cases distinguish missing adapter setup from an actual missing-import request; an early malformed-wire refusal cannot satisfy the ambiguity/staleness controls. Every failed-link case withholds LinkedPackage, and rejected static judgments withhold an evaluable clause. Tests exercise real adapter/checker paths without evaluator calls or filesystem/network authority inferred from path strings. TC-024 varies input order and prior success to expose leaked partial results. Runtime graph traversal and cancellation are outside this test design and remain governed by the accepted profile plus future implementation contracts.
