...
The complete case list is generated from one catalog, while an independent
exhaustive match supplies expected outcomes. Runtime cases require source/report
provenance; the explicitly separate missing-model case requires error provenance.
No acceptance assertion is guarded by the presence of its own output field.
