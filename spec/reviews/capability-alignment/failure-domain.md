...
| FND-010 | low | `quire.capability-kind/v1` does not name the FR-290 revision it denotes. FR-057 mints the version string in QSL, and FR-290 has no version of its own. TC-153 reads a "vendored FR-290 Values table", and FR-057 does not say which revision that copy must be. If FR-290's Values change, v1 could denote two different sets. Fix: state that `quire.capability-kind/v1` denotes exactly the FR-290 Values table at a named QSpec revision, and that TC-153's vendored copy is pinned to that revision. | FR-057 §Admitted vocabulary, §Dependencies · TC-153 step 1 · QSpec FR-290 |
