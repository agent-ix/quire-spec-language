...
| FND-002 | medium | TC-153 step 1 refers to a committed copy of the FR-290 Values table. No copy of FR-290 exists in the QSL tree: `git ls-files` finds none, and `resources/native-v1` holds committed copies of only FR-059, FR-060 and FR-061. Without a pinned source, the set-equality check in FR-057-AC-1 compares the code against a copy of the labels in the test, so it cannot catch drift from FR-290. Fix: in TC-153, name the exact source and revision, for example `quire-specification:spec/objects/protocol/FR-290-protocol-claim-kind.md` at a pinned commit. Also name how the test reads it. Do not use `resources/native-v1/` for this. | FR-057-AC-1, TC-153 |
...
- Checked the QSL tree:
  ...
  - `git ls-files` finds no committed copy of FR-290.
