...
reads from a committed copy of an FR-290 file that does not exist in this repository (FND-003).
...
| FND-003 | medium | TC-153 step 1 reads from a committed copy of the FR-290 Values table, but no copy of FR-290 exists in this repository. `resources/native-v1/` has FR-060 and FR-061 and no FR-290. That tree holds copied content and is not edited here. The test cannot be written as specified. If a copy is added later and refreshed from upstream, the test would follow FR-290 silently and never force the new vocabulary version FR-057 §Dependencies requires. Fix: in TC-153 step 1, compare the admitted set with FR-057's own v1 table, held as a test constant. State that any change to that constant needs a new capability vocabulary version. The FR-290 revision it matches is SR-491 FND-010. | TC-153 step 1 · FR-057-AC-1 · FR-057 §Dependencies |
