...
| Skeleton spine (§1.1, T-2) | High | Medium | It needs M-4, the `replay` facade, QC-1, E7 candidate sets and agent-ix/quire-contract-codegen#86. | No: FND-002 |
...
| External sinks: agent-ix/quire-contract-ir#141 and agent-ix/quire-contract-codegen#86 | Medium | High | Many roles on each, in repositories QSL does not control. OBS-212-1 already shows IR coverage red. | Partly: FND-008 |
...
| FND-008 | medium | Two external tickets carry a large share of the combined architecture. agent-ix/quire-contract-ir#141 holds v2 intake of value, expression and temporal nodes, the S6 enum decode, removal of 27 `as_str()` sites, the string-edge scan and the IR root → QSL edge removal (ADR-011:688; ADR-012:391, 410-411, 888). T-5 still asks whether #141 removes that edge (ADR-011:921). agent-ix/quire-contract-codegen#86 holds the S9 enum, `negotiate_*` over candidate sets and extents, the solver-absence test, the S6 matches and the string-edge scan. #185's exit, #188, #189 and #217 wait on it, and it waits on #141 and agent-ix/quire-specification#134 (ADR-012:881, 913-916). SR-498 OBS-212-1 shows IR coverage red today. The records name no slice plan for either ticket. Fix: in ADR-012 §14.2, split the #86 amendment into bounded children: S9 enum with candidate-set input; solver-absence test; S6 matches after #141. Ask the IR owner to split #141 the same way: v2 intake decode; predicate and temporal admission with the edge removal. | ADR-011:688, 921; ADR-012:391, 410-411, 881, 888, 913-916; SR-498 OBS-212-1 |
...
- FND-002 is Remaining work: agent-ix/quire-contract-codegen#87. #212 does not wait on it (#212 rulings, 2026-09-19,
  SR-502 FND-001).
...
- FND-008 is Remaining work: agent-ix/quire-contract-ir#141 and agent-ix/quire-contract-codegen#86.
