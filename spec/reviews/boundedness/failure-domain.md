...
FND-001 fixed: TR-3 and A-2 follow QSpec FR-090/FR-250/FR-255, no interval under infinite-trace; the grammar note is §13 item 2. FND-002 fixed: fairness in A-2, A-4, `TemporalCounterexample` and the scenario 5 comparison. FND-003 fixed: §8 rule for QSpec FR-346 quantifiers. FND-004 fixed: §4 extent over the transitive closure, recursive types added. FND-005 fixed: classification by limits type; model normalization stays B-2, NFR-012 unchanged; FR-082 amendment narrowed to `TypeEnvironmentLimits`. FND-006 fixed: `DomainKey`, boundable table and request-writer refusals in §4. FND-007 fixed: route guard dropped (Alternatives). FND-008 fixed: three-valued safety-fragment rule in A-4. FND-009 fixed: TR-1/TR-6 use the actual fields. FND-010 fixed: §4 "IR's predicate" and the O-20 row. FND-011 fixed: lasso validity in A-4, TR-2 states `0`. FND-012 fixed: TR-4 names `unsupported_construct`. FND-013 fixed: §7 row added. FND-014 fixed: node count.
