...
Applied in one pass under the #205 coordinator's rulings.

| Finding | Fix |
| --- | --- |
| SR-468 FND-021 (medium) | §1 I2 and §2.2 E3: at E3 a view holds only `WireNodeId`s, and an I2 reference is `PackageNodeKey{package: package_id, node: WireNodeId}` (ADR-013 T-3). Under QC-18 a bare `NodeKey` is unique across packages (ADR-013 O-04). |
| SR-467 FND-014 (medium) | New §4 dependency binding: E4 and `replay` at E9 refuse with `DependencyIdentityMismatch` unless each source-compiled dependency's recomputed `package_id` equals that of the verified view E3 resolved against. Source bytes come from the S4 source resolution for a compile, and from the QC-1 byte provision for `replay`. §1 I2, §2.1 E4 and E9, and §2.3 cite it. |
| SR-470 FND-017 (medium) | T-12 is the single API-surface check: (a) CG calls only the `replay` facade, (b) only `check` calls the `NodeKey` constructor, (c) only `model` calls the `EffectiveId` constructor. §6.1 states (b) and (c), and FB-05 cites part (a). |
| SR-467 FND-015 (low) | §2.1 E8 and the E9 details use `source: ReplaySource { Witness(Witness), Input(values) }` (ADR-013 O-25). An `Input`-sourced replay settles `reproduced-without-witness` and never counts as backend evidence. |
| SR-468 FND-022 (low) | Fixed in sibling ADR (ADR-013). |
| SR-468 FND-023 (low) | Fixed in sibling ADR (ADR-013). |
| SR-468 FND-024 (low) | Fixed in sibling ADR (ADR-013). |
