...
| SR-467 FND-015 (low) | §2.1 E8 and the E9 details use `source: ReplaySource { Witness(Witness), Input(values) }` (ADR-013 O-25, cited as AD-016 amendment ADR-013 QC-20). An `Input`-sourced replay settles `reproduced-without-witness` and never counts as backend evidence. |
