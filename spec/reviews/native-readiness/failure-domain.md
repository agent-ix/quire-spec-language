---
id: SR-021
title: "failure-domain review of native syntax readiness"
type: SpecReview
...
Diagnostic.source remains source identity, never a nested error. Error::source returns None, and Display cannot perform I/O or fail except through its formatter.
...
Usage validation precedes I/O for non-UTF-8 labels and commands; native source errors keep exit 1, missing files keep exit 2 and resource exhaustion keeps exit 3. A non-UTF-8 filesystem path reaches actual I/O without lossy conversion. Formatter append refusal yields no partial String and preserves the format phase and original source location. Malformed-corpus success is allowed only as observed syntax with exact submitted bytes; no parser recovery or evaluator result is invented. No process/server concurrency is introduced.
