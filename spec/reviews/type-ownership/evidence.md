...
| FND-007 | medium | resolved | C-11 names the copied seed vector and a CG widening test at `i64::MIN` and `i64::MAX` (CG #50). It also adds the rule that the join is by parameter node id. Who copies the seed vector into this repo (WP9) is still not named. That is minor. |
