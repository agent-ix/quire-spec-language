...
| TC-243 | Typestate constructors are private to their stage module | Manual | P1 | FR-087-AC-1 | ✅ Passed locally; QSL-158, `xtask::typestate_scan` (steps 1-3 by scan; step 4 follows from every field being private) |
| TC-244 | compile_fail matrix over every forbidden typestate construction (R-10, O-15) | Unit | P1 | FR-087-AC-2 | ✅ Passed locally; QSL-158, step 0 by `xtask::typestate_scan`, rows 1-5 by `compile_fail` doctests (row 6 is row 2's) |
| TC-245 | PackageNodeKey has exactly one shape and declared equality | Unit | P1 | FR-087-AC-5 | ✅ Passed locally |
| TC-246 | ResolvedSourcePackage is retired, with no dangling caller | Integration | P1 | FR-087-AC-7, FR-087-CON-4 | 🚧 Planned; QSL-158: blocked on a spec gap, see FR-087 Status |
| TC-247 | The canonical EmittedPackage/CheckedPackage stay distinct from their pre-existing namesakes | Integration | P1 | FR-087-AC-8, FR-087-AC-10 | ✅ Passed locally; QSL-158, `xtask::typestate_scan` (steps 1-4, 6 and 7; step 5 by the PR diff) |
...
| TC-255 | NodeKey is never minted from a WireNodeId; E4 and E9 resolve by lookup, never by construction | Integration | P1 | FR-087-AC-6 | ✅ Passed locally; QSL-158, `xtask::typestate_scan` (steps 1-5; step 6's transitive trace is not automated, see FR-087 Status) |
...
| TC-258 | QualifiedName is used only as a declared preimage component, never as an identity | Manual | P1 | FR-088-AC-6 | ✅ Passed locally; QSL-158, steps 1-3 by `xtask::typestate_scan`, step 4 by `value::expression::family` |
...
| TC-260 | ValueTypeRef is exactly the two-member union Native/Package | Manual | P1 | FR-088-AC-8 | ✅ Passed locally; QSL-158, `xtask::typestate_scan` (steps 1-3; step 4 follows from the two-variant union) |
...
| TC-281 | value::library and value::package_identity relocate into the new top-level library module, per the R-10/T-3 shapes | Integration | P1 | FR-087-AC-11 | ✅ Passed locally; QSL-158, `xtask::typestate_scan` (steps 1, 4 and 5); step 3 by TC-227; step 6 by `arch-lint`'s T12-B allow-list, which names only `check`; step 2's byte-identity against the pre-relocation tree is not automated |
...
| TC-379 | E3 resolves an imported name to its PackageNodeKey and refuses a missing or ambiguous one | Unit | P1 | FR-087-AC-13 | 🚧 Planned; QSL-158 (PR #299 review, finding 2): blocked on a spec gap, see FR-087 Status |
...
| TC-416 | The v2 emission arm writes the nodes check lowered, and each emitted node recomputes to its node id | Integration | P1 | FR-093-AC-7, FR-093-AC-9, FR-093-AC-12, FR-093-AC-13 | 🚧 Planned; QSL-6 S1b after QSL-156 A4b |
...
test cases. TC-246 and TC-379 are `🚧 Planned`, blocked on a spec gap
FR-087's Status records. TC-259 is partial (QSL-156), and TC-251 covers
FR-088-AC-5's `QualifiedName` half only. The rest pass locally. FR-087 also resolves, by owner
...
and AC-11, and TC-416 AC-7, AC-9, AC-12 (each node's `dependencies`,
