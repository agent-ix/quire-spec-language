...
| TC-411 | A quantity UnitId is a declared unit's node key or a compound unit's digest, and the two never compare equal | Unit | P1 | FR-088-AC-12 | ✅ Passed locally; step 3 runs with `QSPEC_DIR` set |
...
`UnitId`; it passes locally, and its QSpec-vector step runs only with
`QSPEC_DIR` set. FR-084-AC-7 (TC-410) covers one object universe per connected
