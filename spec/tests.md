...
| TC-160 | Every family implements the six-part checked contract with no bypass | Unit | P1 | FR-062-AC-1..FR-062-AC-7, FR-062-AC-9 | ✅ Passed locally for AC-2, AC-5 (hook level), AC-7 backed by TC-378, and AC-12; partly for AC-3 (third clause only, QSL-246 owns the rest), AC-6 (first sentence only, QSL-246 owns the rest) and AC-4 (QSL-140: the no-kind clause passes, the with-kind clause is pending QSL-42); AC-1 owned by QSL-140/QSL-242; AC-9 unbacked (QSL-242) |
...
| TC-253 | The verified binding admits VerifiedPackage only under all three conditions, refusing each failure independently | Integration | P1 | FR-087-AC-3 | ✅ Passed locally; #340 (step 3 and steps 5-7 at the `library` level, see FR-087 Status); step 9's QSpec-fixture read under `make conformance` |
