...
| FR-097-AC-6 | For record types QSL checks and emits, IR's v2 lowering at the pinned revision with `require_bounds` returns `RequiresBound` exactly when QSL's extent is `Unbounded`, and IR's first unbounded node is a form QSL names as a domain. For operation-application requirement records (FR-062-AC-13), QSL's record is the authority for the extent, and the agreement is per application node for each record whose roots are all reachable from the application node through its operands: IR's predicate on that node returns `RequiresBound` exactly when the record's extent is `Unbounded`. In `function f using v(x: Int[0, 9], n: Integer): Integer pure { (x + 1) + n }`, IR's answer for the inner `+` agrees with its `Bounded` record and for the outer `+` with its `Unbounded` record. A record with a root reached through a `let` binder's bound value (FR-062's RR-7 `*`) or read only by a guard (the `+` of `if n = 0 then x + 1 else 0` over `x: Int[0, 9]`, `n: Integer`) is outside the agreement: its extent is the record's, and IR's predicate is not compared. | Test (TC-440) |
