...
Three parts of O-12 and C-21 are not built, for one reason: each needs a
`RawSourceRef` for a source QSL reads itself, and QSL's `SourceIdentity`
(FR-001) carries an identity and a one-string revision, with no authority
and no revision namespace. The CLI grammar (FR-010) takes
`<source-id> <source-revision> <file>`. No requirement states where a
QSL-read source's authority or revision namespace comes from. The three
