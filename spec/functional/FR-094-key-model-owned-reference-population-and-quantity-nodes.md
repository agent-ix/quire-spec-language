...
content: the lock selects the domain package's bytes by digest, and one
version of an identity per check. An empty body also keeps model declaration
...
`check` builds the node of each model declaration that a checked node
references: the target of a `Reference<T>` type, the declaring node of a
`field`, `operation` or `relationship_end` member, and a `type_argument`
member's type. A field, operation or relationship-end member has no node of
...
normalization rules' derivation facts.
...
(ADR-013 O-04, C-02). The correspondence holds exactly the pairs `check`
keyed: each `DeclarationKey` it holds has one node key, and each node key one
...
`check` SHALL build these `StateModel` type nodes:
...
call reaches it. `check::checked_dispatch` builds three per dispatch
candidate: an operation's authored precondition, its effective precondition,
and its body. `check` SHALL build a clause function's node as FR-092's
function node, with these differences:

- `owner`: the `ModelOwner` of the operation member whose clause it is. For an
  authored precondition, that is the member that authors it. For an effective
  precondition and for a body, it is the dispatch candidate.
- `declaration`: `null`, as for every model-owned node.
- body: FR-092's `parameters`, `body` and `decreases` bindings, followed by
  binding `clause` = the clause kind as a text literal typed at the text
  scalar node (FR-092 T3): `"precondition"` for an authored or effective
  precondition, `"body"` for a body, and `"invariant"` for an invariant.
...
member's precondition and body apart.
...
`check` reads a compound unit's terms from the `CompoundUnit` that the
package's unit table holds for its `UnitId`. A declared unit `u` and the
...
- a compound `UnitId` that the package's unit table does not hold.

Type admission refuses the first two for a source-compiled unit before `check`
keys a node (FR-091's unresolved model reference, `TypeEnvironment::new`'s
`UnknownObjectType`), and the unit table admits every compound unit a checked
quantity type names, so reaching one is a fault in `check`, not in the input.
...
`size(): Integer`, object type `ix://acme/orders/Invoice`, and relationship
`ix://acme/orders/billedTo`. `M::Order` names that `Order` object type. A
...
| --- | --- | --- | --- |
| FR-094-CON-1 | The `match` that builds a model declaration node from a domain package record kind, the `match` that builds a quantity type node from a `UnitId` domain, and the `match` that spells a clause kind have no `_` or catch-all arm (ADR-012 §5.1). | Design | Test (TC-417, TC-418, TC-419) |
...
| FR-094-AC-3 | In functions over the parameters `p: Population<M::Order>[3]` and `r: Reference<M::Order>`, in that order, `p`'s parameter node keys to P5 over type node PO1, whose `semantic_type` is S1, and `r`'s to P6. `allInstances<M::Order>(p)` keys to E4 with `result_type` S2, `lookup<M::Order>(p, r) absent undefined` to E5 with `result_type` R1, and `lookup<M::Order>(p, r) absent empty` to E6 with `result_type` R3. `p: Population<M::Order>[5]` has type node PO2. | Test (TC-417) |
| FR-094-AC-4 | With `r: Reference<M::Order>` at level 1, `deref(r).total` lowers to E7 and E8, whose `field` member names M1 and `total`, and `r.size()` keys to E9, whose `operation` member names M1 and `size`. When object type `Sub` has supertype `Order` and inherits `total` and `size`, `deref(s).total` and `s.size()` over `s: Reference<M::Sub>` name `Sub`'s model node, not M1. | Test (TC-417) |
| FR-094-AC-5 | `checked_dispatch_operation` for `Order.size` with the authored precondition `true` and the body `7`, and receiver parameter `self: Reference<M::Order>` (P7), builds clause functions keyed C1 and C2. Under `acme/orders` `2.0.0` the precondition keys to C3. The same precondition authored on `Order.count` keys to C4. Each preimage carries `ModelOwner` and `declaration` `null`, and no preimage contains the synthesized function name. | Test (TC-418) |
| FR-094-AC-6 | A parameter typed as a quantity of QSpec's declared unit `metre` has semantic type `79637623a46d29e884b62c6fa292aeb29d41e4ecc4e800b4d7ee910a3eaf23a4`, QSpec's `unit-metre` key, and the graph holds no `compound_unit` node for it. The compound units `metre^2`, `metre^1 second^-1`, the dimensionless unit and `metre^1` key to U1, U2, U3 and U4. U2's first term names metre, and U4 differs from the `metre` unit key. | Test (TC-419) |
| FR-094-AC-7 | Keying a `Reference` whose `EffectiveId` is no `type_identities` value, a `DeclarationKey` whose `package` is no admitted identity, and a compound `UnitId` absent from the unit table each refuses as an internal fault naming that value, and yields no key. | Test (TC-417, TC-419) |
...
- [ADR-013](../decisions/ADR-013-canonical-type-package-conversion-ownership.md)
  O-01, O-03, O-04, O-05, O-06, C-02, OQ-B, OQ-G, T-4, and QC-25 and QC-26,
  the QSpec requests this requirement makes.
...
- `checked_dispatch_operation` names its clause functions
  ...
  `<candidate node>`, and builds no key for them.
