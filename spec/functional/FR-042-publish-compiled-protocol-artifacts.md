...
| FR-042-AC-15 | `protocol_artifact::handoff::write_v1(directory)`, in the normal (non-dev) dependency surface behind the `handoff-writer` feature and adding no dependency, lets a downstream crate compile the authored recipe fresh and write a complete handoff -- original sources, model source, every selected dependency's exact bytes, the independent `expected.json` selection, the artifact reference and a `SHA256SUMS` covering every other file -- to a caller-chosen new directory, in-process, without building this crate's examples or dev-dependencies. It reads the emitted bytes back through the strict public reader before writing anything. Two calls write byte-identical trees; an existing directory refuses without touching its contents. | Test (TC-121) |
