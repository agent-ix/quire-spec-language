...
| FR-027-AC-2 | A source-only request compiles without runtime files; native-run/1 and unexpected runtime fields refuse at the command boundary. | Test |
| FR-027-AC-3 | Stale selected source and malformed syntax return original codes with empty stdout; existing run and parse/format tests still pass. | Test |
