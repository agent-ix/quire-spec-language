...
selects no models and no clause bindings. A spine refusal uses the same
command-error envelope, with the refusing spine stage (`source`, `forms`,
`assembly`, `check` or `emit`) as its stage and that stage's cause code as its
...
from its header, after the source's digest check and before either compiler
runs. Each program source goes to exactly one compiler. If the header declares
...
| FR-027-AC-7 | A program source declaring any other edition refuses with `unknown_edition`, exit 20, empty stdout, and a message naming the file and the edition. A `1-draft` request selecting models or clause bindings refuses with `invalid-request`, exit 20. | Test (TC-435) |
