...
follow-up request: it supplies one `FiniteBound` for each of the item's
unbounded `DomainKey`s, and `RequestWriter::bounded_item` writes the
follow-up with its own request index (ADR-014 §4 "Bounded request"). It
