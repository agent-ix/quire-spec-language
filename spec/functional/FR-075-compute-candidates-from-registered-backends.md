...
lowering-target selector. Replacing that catalog with a backend chosen by
`BackendId` and resolved in `route` is
[#217](https://github.com/agent-ix/quire-spec-language/issues/217)'s work,
with SEAM-1 (ADR-011 §6.2, the `lowering` row; §7.3 M-6b).
...
the lowering-target selector; its replacement is #217's (Description).
