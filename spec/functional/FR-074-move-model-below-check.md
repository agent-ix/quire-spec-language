...
`format_interval` and `CLAUSE_SELF_SLOT`) into `check`. The relocated
obligation reads the model layer's shared index, `model::index::ModelIndex`
(`pub`), through its `pub(crate)` `field`, `operations` and `scalar_bounds`
accessors, and calls back into `model::conformance::missing_member`
(`pub(crate)`), `AxisFailure` and `ConformanceOutcome` (both `pub`). It does
so from `check`, which now sits above `model` in the layer-3 order: that
...
- `qsl-semantics/src/model/conformance.rs:763-1094`: the contiguous
  ...
  required (`:89-100`).
- `model::index::ModelIndex`, the shared per-package index, and
  `model::conformance`'s `missing_member` function: the state and helper the
  relocated obligation reads back into once it moves.
...
- `model::conformance::missing_member` visible to `check` as `pub(crate)`,
  `model::conformance::AxisFailure` and `model::conformance::ConformanceOutcome`
  `pub`, and `model::index::ModelIndex` `pub` with its `field`, `operations`
  and `scalar_bounds` accessors `pub(crate)` — no other `model::conformance`
  item is visible outside `model`, since the compiler does not require it.
...
`model::index::ModelIndex` — after they move to `check`.
...
### The model surface `check` reads is exactly what the obligation needs

The relocated `check_field_refinement_obligation` reads `model::index` and
`model::conformance` through exactly these items (it also reads the
domain-package, key and refusal types named in the section above):

- `model::index::ModelIndex`, `pub`, which callers build once per domain
  package and pass to the obligation, and its `field`, `operations` and
  `scalar_bounds` accessors, `pub(crate)`;
- `model::conformance::missing_member`, `pub(crate)`;
- `model::conformance::AxisFailure` and `model::conformance::ConformanceOutcome`,
  `pub`.

Every other `ModelIndex` accessor, and every `ModelIndex` field, is either
private or `pub(crate)` for the model layer's own use; `check` calls none of
them. `model::conformance`'s `charge_axis` is private and
`multiplicity_conforms` is `pub(super)`: the relocated obligation calls
neither, so making them visible to `check` would be speculative, not
compiler-required.
...
| FR-074-AC-4 | `model::index::ModelIndex` is `pub` and its `field`, `operations` and `scalar_bounds` accessors are `pub(crate)`; `model::conformance::missing_member` is `pub(crate)`; `AxisFailure`/`ConformanceOutcome` are `pub`; `model::conformance`'s `charge_axis` is private and `multiplicity_conforms` is `pub(super)`. Verified by inspecting the visibility qualifiers in `model/index.rs` and `model/conformance.rs`, and the `crate::model` paths `check/field_refinement.rs` imports. | Inspection |
