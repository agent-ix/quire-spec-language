...
`format_interval` and `CLAUSE_SELF_SLOT`) into `check`, widening exactly
four `model::conformance` items — `ConformanceIndex` (the struct and its
`build` constructor), `AxisFailure`, `ConformanceOutcome` and
`missing_member` — to `pub(crate)` so the relocated code can call back into
them from `check`, which now sits above `model` in the layer-3 order: that
...
### Widening is exactly four items, no more

`ConformanceIndex` (struct and `build`), `AxisFailure`, `ConformanceOutcome`
and `missing_member` are the only `model::conformance` items this
requirement widens to `pub(crate)`. `AxisFailure` and `ConformanceOutcome`
were already `pub`; only `ConformanceIndex`, `ConformanceIndex::build` and
`missing_member` change visibility. No other `model::conformance` item
(`walk_ancestors`, `charge_axis`, `MAX_CONFORMANCE_DEPTH`,
`generals_by_specific`, `type_conforms`, `value_type_conforms`,
`multiplicity_conforms`) is widened: the relocated
