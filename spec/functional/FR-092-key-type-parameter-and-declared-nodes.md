...
ADR-013 O-04 assigns this preimage to QSL ("QSL implements a proposed one,
specified with the type-node key work"). It follows the ADR-013 OQ-G ruling
...
| `owner` | Present exactly when `declaration` is not `null`: the declaring unit's `SourceOwner` as `{kind: "source", authority, identity}`, the QSpec `SourceOwner` shape. Absent otherwise. |
...
`size` and `ordinal` and an `operation.member` `position`. A literal's value
...
The walk that builds a preimage is bounded by the check stage's depth limit
(`MAX_CHECKING_DEPTH`, ADR-011 §2.3). A body nested deeper refuses with a
depth refusal and yields no key. An empty `qualified_name`, an empty binding
name or an empty `semantic_form` refuses and yields no key.
...
builtin `Integer` scalar node for an integer and the builtin text scalar node
for a text value.
...
| a declared `record R { f: T; ... }` | `composite_type` / `record`, with `declaration` and `owner` | itself | one binding per field in declaration order, `f` = `reference(T)`; an optional field `f?: T` references the `Option<T>` node |
...
- body: `aggregate{[binding "name" = the name as a text literal, binding
  "level" = the binder's level as an integer literal]}`.
...
| FR-092-AC-7 | A checked type or value body nested deeper than `MAX_CHECKING_DEPTH` terms refuses with the depth refusal and yields no key; a body exactly at the limit is keyed. A declaration whose qualified name has no segment refuses and yields no key. | Test (TC-413) |
