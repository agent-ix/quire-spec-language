...
`self-test`, `review`, `roles` and `rule-syntax`.
...
or I/O failure, and 3 for resource incompleteness.
Errors implement the standard Rust Display and Error traits and have an owned
stable-code catalog. No evaluator or semantic matcher is claimed by a
byte/syntax check.
...
shared-reference decoder or a model source.
...
No audit launches Python, Node, a shell or another external process.
...
| FR-012-AC-8 | Audits leave selected artifact bytes unchanged and identify byte/syntax checks without claiming evaluation. | Test |
