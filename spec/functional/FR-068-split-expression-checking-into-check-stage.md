...
- A top-level `check` module of the layer-3 crate `qsl-semantics`
  (`qsl-semantics/src/check/`, declared in `qsl-semantics/src/lib.rs` beside
  `family`, `library`, `model` and `value`) at the layer-3 position ADR-011
