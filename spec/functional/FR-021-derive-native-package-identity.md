...
The manifest's source identity carries the four source labels of FR-001, so the independently authored hash vectors of FR-021-AC-1 change. Implemented under QSL-233 (ADR-013 §7 S-4b); the `minimal`, `controls` and `multiple` vectors were regenerated from the independent recipe with authority `agent-ix` and revision namespace `draft`.
