...
type is `NodeKey`, `DeclarationKey`, or any other type ADR-013 O-03, O-04 or
O-07 defines as a check-time-minted identity (`DeclarationKey` is ADR-013
O-03, per ADR-011 §2.2's E3 row). Identity is minted only at check (S3); a
form built at S2 SHALL carry no identity at all, not a placeholder or a
provisional one.
...
every Rust source and build-input file — `src/`, `tests/`, `benches/`,
`xtask/` and `examples/` — not only `src/`. This scan SHALL NOT extend to
`spec/` or `docs/`, which name these three symbols by design (this
requirement's own artifacts, and ADR-010/ADR-011's historical record of the
seam, among them) and are never a compatibility hole this requirement needs
to close. The pre-migration test that
...
| FR-067-AC-7 | A parsed form's edition equals the edition its originating CST recorded; a test that builds two parsed forms from CSTs recorded under two different literal edition values (`"0-draft"` and `"1-draft"`) and reads each form's edition back observes each form's edition matching the CST it was built from — the `"0-draft"` CST's form reads `"0-draft"`, the `"1-draft"` CST's form reads `"1-draft"` — not one value shared by both and not swapped, showing the edition is carried from the CST rather than hardcoded, defaulted or transposed. | Test (TC-167) |
| FR-067-AC-8 | For a construct that declares a bound or an extent, the resulting parsed form carries that declared value unchanged; a test that declares two different bound or extent values on two otherwise-identical constructs and reads each resulting form's carried value back observes each form's value matching the construct it was built from, not one value shared by both and not swapped, showing the value is carried from the declaration rather than a shared default. | Test (TC-167) |
| FR-067-AC-9 | After this requirement's implementation, `value::expression::syntax` does not exist as a module, and the `Expression` enum, `FunctionDeclaration`, `BinaryOperator`, `ClauseKind`, `DeclaredClauseKind`, `FieldInitializer`, `BinderQuery` and `Accumulation` are each defined exactly once, under `forms`. A module-tree scan confirms `value::expression::syntax`'s absence; a scan of type definitions confirms none of the eight names has a second, independent definition remaining under `value::expression` or anywhere else. `value::expression`'s own code importing these types from `forms` does not itself fail this criterion; a second, independent definition of any one of them does. | Test (TC-169) |
