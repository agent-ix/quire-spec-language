...
- IR-242: the IR reader's recursion preimage. It derives an in-group
  ordinal from graph order, where FR-092 orders a group by content (ADR-013
  QC-24).
...
Specified under QSL-208. Implemented on the QSL-156 slice A4b branch
(`task/156-a4b-application-key`), pending merge: `check` lowers each checked
node in `qsl-semantics/src/check/lowering.rs` and keys it by FR-092, and
TC-415 backs AC-1 to AC-6, AC-8 and CON-1 there. A4b spells an integer
