...
| FR-093-AC-17 | `qsl_package::diagnostics_catalog` is a public accessor for the `CheckedArtifactRef` the v2 emission arm writes at `diagnostics.catalog`: QSpec's `quire.native.diagnostics/v1` at revision `1-draft.7`. A caller that needs it as evidence (IR's checked-package v2 reader refuses it as a stale dependency without one) reads it from this accessor instead of parsing it back out of the emitted bytes. `diagnostics_catalog()` equals the emitted `diagnostics.catalog` reference member for member. | Test (TC-416) |
