...
evaluation outcome, a typed family refusal that carries a catalog code, or a
family-owned evaluation result that carries its own code or undefined reason
**So that** I can tell a kernel result (completed, undefined, refused,
incomplete) from a family that does not evaluate natively, from a family that
ran and refused or found no value, and from a broken evaluator invariant, by matching a closed type rather than by reading a
...
FamilyEvaluated(FamilyResult) }`, beside T-4's `InternalFault`. The kernel
...
- **Then** the caller receives `FamilyOutcome::FamilyEvaluated` holding a
  ...
  `refusal`, distinct from a family that did not run.
