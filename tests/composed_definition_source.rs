...
//! TC-114: fixed native definition identities, revisions and their declared
//! normative rule inventories. QSL resolves a registered definition by
//! reference (PLAT-887): this module owns no vendored document bytes, so
//! these tests compare identity, revision, path and rule-inventory metadata,
//! never content. Supplied-inventory refusal and declaration admission are
//! tested separately.
...
fn exact_definition_labels_select_distinct_registered_meanings() {
    assert_eq!(
    ...
    for row in EXPECTED {
        ...
        // Recognition is by identity alone; distinct registered meanings
...
#[test]
#[trace("TC-114", "FR-036-AC-1")]
fn direct_rules_retain_the_selected_file_inventory() {
    ...
    for (definition, paths) in expected {
        ...
        // A rule's path is its identity in the inventory (PLAT-887); no two
        // direct rules of one definition may claim the same path.
    ...
    // FR-061 is one shared interface across four direct selections; a
    // different consumer's entry must still name the same rule identity.
    assert_eq!(
